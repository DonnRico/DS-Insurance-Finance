...
Conf.Int<-
  function(net,alpha=0.05,tikhonov=0.0001,dxprop=1e-5,h=1e-5)
  {
    ...
    for (ct in 1:nw)
    {
       ...
       nnmo               <- compute(nn.minus,nn.minus$covariate)
       nnpo               <- compute(nn.plus ,nn.plus$covariate)
...
logLike.EDF<-
  function (x) 
  {
    ...
    netOutput  <- compute(net,covariate,response, exposure)
